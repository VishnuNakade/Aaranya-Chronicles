...
test('movement, jump, coins, combat, damage, completion and cleanup', async ({ page }) => {
  ...
  await expect(page.locator('.coin-count')).toContainText('1 / 8');
  ...
  await page.keyboard.down('j'); await page.waitForTimeout(80); await page.keyboard.up('j');
  ...
  await expect(page.getByText('The first light is yours.')).toBeVisible();
  expect(await page.evaluate(() => JSON.parse(localStorage.getItem('aaranya:v1')).completed)).toBe(true);
  await page.getByRole('button', { name: 'Play again' }).click(); await expect(page.getByLabel('3 hearts')).toBeVisible();
